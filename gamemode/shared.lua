GM.Name 	= "Deathrun 死亡跑酷"
GM.Author 	= "Arizard"
GM.Version	= "Release 1.0"
...
function GM:CreateTeams()
	team.SetUp(TEAM_GHOST, "幽灵", DR.Colors.GhostTeam, false)
	team.SetUp(TEAM_RUNNER, "奔跑者", DR.Colors.RunnerTeam, false)
	team.SetUp(TEAM_DEATH, "死神", DR.Colors.DeathTeam, false)
...
-- if SERVER then
-- 	hook.Add( "SetupMove", "Bhop.AntiCheat", function( ply, move )
-- 		if bit.band( move:GetButtons(), IN_JUMP ) ~= 0 and bit.band( move:GetOldButtons(), IN_JUMP ) == 0 then
-- 			if bit.band( move:GetButtons(), IN_LEFT ) ~= 0 and bit.band( move:GetButtons(), IN_MOVELEFT ) ~= 0 then
-- 				ply:Kick( "您因为同时使用+moveleft和+left而被踢出。请不要使用脚本等辅助工具！" )
-- 				PrintMessage( HUD_PRINTTALK, ply:Name().. " 疑似使用脚本作弊，已被踢出。" )
-- 			end
-- 			if bit.band( move:GetButtons(), IN_RIGHT ) ~= 0 and bit.band( move:GetButtons(), IN_MOVERIGHT ) ~= 0 then
-- 				ply:Kick( "您因为同时使用+moveright和+right而被踢出。请不要使用脚本等辅助工具！" )
-- 				PrintMessage( HUD_PRINTTALK, ply:Name().. " 疑似使用脚本作弊，已被踢出。" )
-- 			end
-- 		end
-- 	end)
...
if CLIENT then
	...
	hook.Add("HUDPaint", "SendSpectateConVarInfo", function()
		...
		if GetConVarNumber( "deathrun_spectate_only" ) == 1 then
			DR:OpenForcedSpectatorMenu( [[您现在正在观察者队伍。
				您可以点击下列按钮，
				或去往F2选项以设置观察者模式。
				\n\n您想回到玩家队伍么？]] )
		end
